...
*Ecuación 2.1.1*\
#kw[Función de una variable aleatoria]. Si $X$ es una variable aleatoria con fds $F_X (x)$, entonces cualquier función de X, digamos g(X) también es una variable aleatoria.
$
  P(Y in A) = P(g(X) in A)
$
...
$cal(X)$ espacio muestral de $X$\
$cal(Y)$ espacio muestral de $Y$\
...
Sea $X$ con una fda $F_X (x)$, sea $Y = g(X)$, y sean $cal(X)$ e $cal(Y)$ definidas como $cal(X) = \{x : f_X (x) > 0\} "  y  " cal(Y) = \{y : y = g(x) " para algunos " x in cal(X)\}.$.
...
Sea $X$ tal que tiene una fdp $f_X (x)$ y sea $Y = g(X)$, donde $g$ es una función monótona. Sea $cal(X)$ y $cal(Y)$ definidos como en (2.1.3). Tenemos que $f_X (x)$ es continua en $cal(X)$ y que $g^(-1)(y)$ tiene una derivada continua en $cal(Y)$. Entonces la fdp en $Y$ es dada por.
